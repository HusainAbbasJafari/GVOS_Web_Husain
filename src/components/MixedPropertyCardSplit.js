...
const MixedPropertyCardSplit = ({ item, activeMarker }) => {
    ...
    return (
        <div
            className={`h-full flex flex-col border hover:border-primary ${(item.id === activeMarker) ? "border-primary" : "border-zinc-150"} hover:shadow-xl transition-all duration-300 rounded-3xl overflow-hidden`} id={`property_${item.id}`}>
            <div className="px-2 xl:px-3 pt-2 xl:pt-3">
                <div className="relative rounded-2xl">
                    <div className="absolute top-2 right-2 w-8 h-8 z-20 bg-white/80 flex items-center justify-center rounded-full">
                        <Image width={20} height={20} src={`/images/${item.type === 'new home' ? "new_homes.svg" : "resale_icon.svg"}`} alt="house-icon" />
                    </div>
            ...
            <div className="grow flex flex-col justify-between">
                <div className="px-2 xl:px-3 py-2 flex justify-between gap-3 h-[75px]">
                    <div className="flex flex-col justify-between items-center grow">
                        <div className={`grid ${item.type !== 'resale' && 'text-center'}`}>
                            <h3 className={`font-medium line-clamp-1 text-center ${(item.id === activeMarker) ? "text-primary" : "text-dark"}`}>{item.geo_level_1}</h3>
                            <span className={`text-xs flex flex-wrap justify-center items-center gap-1 lh-1 ${(item.id === activeMarker) ? "text-primary" : "text-dark"}`}>
                                {item.geo_level_2}
                ...
                <div className="px-2 xl:px-3 pb-2 xl:pb-2 pt-2">
                    <div className="flex items-center justify-between">
                        ...
                        >
                            <Button
                                size={"sm"}
                                className="!rounded-full text-sm font-semibold px-3 py-2"
                            >
                                {t('viewDetails')}
                            </Button>
                        </Link>
...
export default MixedPropertyCardSplit;
